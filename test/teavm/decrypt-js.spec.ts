...
describe('teavm/decrypt-js', () => {
  it('armor error', async () => {
    ...
    await expect(
      async () => await decrypt(key, fixMessage(msg), { runtime: 'js-java' }),
  ...
  it.skipIf(process.env.CI)(
    ...
    async () => {
      ...
      const decrypted = await decrypt(key, fixMessage(msg), {
        runtime: 'js-java',
      });
  ...
  it('works with pgp 2.4 and ecc', async () => {
    ...
    const decrypted = await decrypt(key, msg, { runtime: 'js-java' });
