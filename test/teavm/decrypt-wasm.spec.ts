...
// const TeaVM: (typeof globalThis)['TeaVM'] = {} as any;
...
describe('teavm/decrypt-wasm', async () => {
  const teavm = await globalThis.TeaVM.wasmGC.load(
    new URL(`../../dist/teavm/lib.wasm`, import.meta.url),
  );

  const decrypt = teavm.exports.decrypt;
  ...
  it('armor error', async () => {
    ...
    const fn = decrypt(key, fixMessage(msg));
    expect(() => fn).toThrow('invalid armor');
  });
